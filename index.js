...
const start = (piyo = new Client()) => {
    ...
    console.log(color(figlet.textSync('Numa Bot', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Numapu', 'yellow'))
        ...
        if (groups.length > groupLimit) {
            ...
            if (chat.groupMetadata.participants.length < memberLimit) {
                ...
                await piyo.simulateTyping(chat.id, true).then(async () => {
                    await piyo.sendText(chat.id, `Hai Cuy, Saya Whatsapp Bot Pintar.  Untuk memulai bot silahkan ketik ${prefix}menu`)
                })
            }
        }
    })
