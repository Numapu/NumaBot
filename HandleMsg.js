...
const notice = ['NumaBot', 'NumaBot']
const { getUser, getPost, searchUser } = require('./lib/Insta')
const path = require('path')
const bent = require('bent')
const Math_js = require('mathjs')
const fs = require('fs-extra')
const errorImg = 'http://numapa.site/bot/error.png'
...
module.exports = HandleMsg = async (piyo, message) => {
    try {
        ...
        const nadhirasayang = './media/nuqibot.jpg'
        ...
        const errorur121 = 'http://numapa.site/bot/error.png'
        ...
        if (chats == 'ga') {
            if (!usermp3) return
            await piyo.sendFile(from, `./media/${sender.id}.mp3`, 'numa.mp3')
            fs.unlinkSync(`./media/${sender.id}.mp3`)
            usermp3.splice(sender.id)
            fs.writeFileSync('./settings/usermp3.json', JSON.stringify(usermp3, null, 2))
        }
        ...
        function isStickerMsg(id) {
            ...
            for (let i of stickerspam) {
                if (i.id === id) {
                    if (i.msg >= 5) {
                        found === true
                        piyo.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Numabot', message.id).then(() => {
        ...
        if (!gg && !hh) {
            switch (command) {
                ...
                case 'speed':
                case 'ping':
                    await piyo.sendText(from, `Speed: ${processTime(t, moment())} _Second_`)
                    break
                case 'tnc':
                ...
                case 'menupenting':
                    ...
                    break
                case 'help':
                    await piyo.sendFile(from, nadhirasayang, 'numa.jpg', menuId.textmenubaru(pushname))
                    break
                case 'menubaru':
                    await piyo.sendFile(from, nadhirasayang, 'numa.jpg', menuId.textmenubaru(pushname))
                    break
                case 'setpictmenu':
                    ...
                    fs.writeFileSync(`./media/nuqibot.jpg`, set)
                ...
                case 'menu':
                    ...
                    let tod = `${timee}`;
                    await piyo.sendFile(from, nadhirasayang, 'numa.jpg', menuId.textmenubaru(pushname, premi, _registered, coloo, tod))
                    break
                case 'rules':
                case 'rule':
                ...
                case 'ownerbot':
                    ...
                    break
                case 'bal':
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user NumaBot, untuk pendaftaran bisa menggunakan /register nama | Jenis Kelamin. Contoh: /register ${pushname}|cewe`, id)
                ...
                case 'stiker':
                    if (isMedia && isImage || isQuotedImage) {
                        try {
                            ...
                            const author = 'NumaBot'
                            const pack = '62881026880515'
                            const mediaData = await decryptMedia(encryptMedia, uaOverride)
                ...
                case '8bit':
                    ...
                    // await piyo.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/8bit/?text1=${bit1}&text2=${bit2}` , 'numa.jpg' , 'Nih Kak' , id)
                    break
                case 'shadow':
                    ...
                    await piyo.reply(from, `Maintance`, id)
                    // await piyo.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/shadowtext/?text=${q}` , 'numa.jpg' , 'Nih Kak' , id)
                    break
                case 'burn':
                ...
                case 'blackpink':
                    ...
                    break
                case 'pornhub':
                    if (args.length == 2) return piyo.reply(from, `Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh */pornhub |Asmar|Numabot*`, id)
                    ...
                    if (argz.length >= 2) {
                        ...
                        await piyo.reply(from, `Wrong Format!\n[❗] Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh */pornhub |loli|animehj*`, id)
                    }
                    break
                case 'mlmaker':
                ...
                case 'bannerff':
                    if (isLimit(serial)) return piyo.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    if (!q) return piyo.reply(from, `Kirim perintah /bannerff [ |Teks1|Teks2 ], contoh /bannerff |Asmar|Kemren sangad`, id)
                    argz = body.trim().split('|')
                    if (argz.length >= 2) {
                        piyo.reply(from, ind.wait(), id)
                        ...
                        await piyo.reply(from, `Wrong Format!\n[❗] Kirim perintah /bannerff [ |Teks1|Teks2 ], contoh /bannerff |Asmar|Kemren sangad`, id)
                    }
                    break
                case 'gmlogo':
                    ...
                    break
                case 'wolf2':
                    if (args.length === 0) return piyo.reply(from, `Kirim perintah *${prefix}wolf2 [ |Teks1|Teks2 ]*, contoh *${prefix}wolf2 |figo|icikiwir*`, id)
                    argz = body.trim().split('|')
                    if (argz.length === 3) {
                        ...
                        await piyo.reply(from, `Wrong Formt!\n[❗] Kirim perintah *${prefix}wolf2 [ |Teks1|Teks2 ]*, contoh *${prefix}wolf2 |Numa|Bot*`, id)
                    }
                    break
                case 'wolf':
                    if (args.length === 0) return piyo.reply(from, `Kirim perintah *${prefix}wolf [ |Teks1|Teks2 ]*, contoh *${prefix}wolf |figo|icikiwir*`, id)
                    argz = body.trim().split('|')
                    if (argz.length === 3) {
                        piyo.reply(from, 'Tunggu sebentar dan pastikan format yang anda masukkan benar', id)
                        const wolf1 = argz[1]
                    ...
                    break
                case 'ninja':
                    if (args.length === 0) return piyo.reply(from, `Kirim perintah *${prefix}ninja [ |Teks1|Teks2 ]*, contoh *${prefix}ninja |figo|icikiwir*`, id)
                    argz = body.trim().split('|')
                    if (argz.length === 3) {
                        piyo.reply(from, 'Tunggu sebentar dan pastikan format yang anda masukkan benar', id)
                        const ninja1 = argz[1]
                        ...
                        await piyo.reply(from, `Wrong Format!\n[❗] Kirim perintah *${prefix}ninja [ |Teks1|Teks2 ]*, contoh *${prefix}ninja |figo|icikiwir*`, id)
                    }
                    break
                case 'galaxytext':
                    if (args.length == 0) return piyo.reply(from, `Membuat Text jadi Gambar galaxy\nPemakaian: ${prefix}galaxytext [teks]\n\ncontoh: ${prefix}galaxytext NumaBot`, id)
                ...
                case 'logo':
                    ...
                    break
                case 'pornhub':
                    if (args.length == 2) return piyo.reply(from, `Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh */pornhub |loli|animehj*`, id)
                    argz = body.trim().split('|')
                    if (argz.length >= 2) {
                        piyo.reply(from, ind.wait(), id)
                        const lpornhub = argz[1]
                ...
                case 'trumptweet':
                    if (args.length == 0) return piyo.reply(from, `Kirim perintah /trumptweet [ Teks ], contoh /trumptweet NumaBot`, id)
                    piyo.reply(from, ind.wait(), id)
                    ...
                    piyo.sendFileFromUrl(from, tumh.message, `Nekonime${ext}`, 'Tuh Banh', id)
                    await limitAdd(serial)
                    break
                //////////////////////////////////////////////////MENU GRUP////////////////////////////////////////////////////////
                case 'afk':
                ...
                case 'antisticker':
                    ...
                    break
                case 'antiporn': //ini antipornnya beli dulu apikenya, cuma support gambar / kalo mau support sticker dan video bisa ke saya , wa.me/62881026880515
                ...
                case 'stopkuiz':
                    ...
                    try {
                        ...
                        fs.writeFileSync('./settings/kuis.json', JSON.stringify(kuis, null, 2))
                        piyo.sendFileFromUrl(from, 'https://i.imgur.com/qkIGnR1.jpg', 'numa.jpg', 'Yah kok udahan sih :(\nNanti Main Kuis Lagi Yak', id)
                    } catch (err) {
                    }
                    break
                ...
                case 'gantiprofile':
                    ...
                    if (isMedia && type == 'image' || isQuotedImage) {
                        ...
                        await piyo.sendText(from, `Sudah Onii Chan >_<`, id)
                    }
                    break
                case 'mystat': {
                    ...
                    if (isOwnerBot) {
                        await piyo.joinGroupViaLink(linkgrup)
                            .then(async () => {
                                ...
                                await piyo.sendText(chekgrup.id, `Halo Cuy, Im NumaBot. Untuk Memulai Bot silahkan ketik  ${prefix}menu`)
                            })
                    } else {
                        ...
                        await piyo.sendContact(from, ownerNumber)
                        if (cgrup.size < memberLimit) return piyo.reply(from, `Apanih member dikit bat ${memberLimit} people`, id)
                        await piyo.joinGroupViaLink(linkgrup)
                            .then(async () => {
                ...
                case 'gayy':
                    if (isMedia && type === 'image' || isQuotedImage) {
                        ...
                        await piyo.reply(from, ind.wait(), id)
                        await piyo.sendFileFromUrl(from, `https://some-random-api.ml/canvas/gay?avatar=${fotogay}`, 'gay.jpg', 'nih coy', id)
                ...
                case 'whatanime':
                    if (isMedia && type === 'image' || isQuotedImage) {
                        ...
                        await piyo.sendFileFromUrl(from, anj.video, 'numa.mp4', `*Terdeteksi Anime Berikut :*\n*Nama :* ${anj.title_romaji}\n*Nama Lain :* ${anj.title_native}\n*DiWaktu :* ${anj.at}\n*Episode :* ${anj.episode}\n*Kesamaan :* ${anj.similarity}`, id)
                    }
                    break
                ...
                case 'genshin': {
                    ...
                    fox += "\n*Title :* " + asu.title
                    fox += "\n*Info :* " + asu.intro
                    await piyo.sendFileFromUrl(from, asu.cover1, 'numa.jpg', fox, id)
                    await piyo.sendFileFromUrl(from, asu.cv[0].audio[2], 'numa.mp3', 'nih kak', id)
                }
                    break
                ...
                case 'nuliskanan': {
                    ...
                    spawn('convert', [
                        ...
                        .on('exit', () => {
                            piyo.sendImage(from, './media/images/buku/setelahkanan.jpg', 'after.jpg', `Udah cuy, donasi dong buat biaya server. bales /donasi untuk melihat cara donasi\nDitulis selama: ${processTime(t, moment())} _detik_`, id)
                        })
                }
                    break
                case 'nuliskiri':
                    ...
                    spawn('convert', [
                        ...
                        .on('error', () => piyo.reply(from, 'Error gan', id))
                        .on('exit', () => {
                            piyo.sendImage(from, './media/images/buku/setelahkiri.jpg', 'after.jpg', `Udah cuy, donasi dong buat biaya server. bales /donasi untuk melihat cara donasi\nDitulis selama: ${processTime(t, moment())} _detik_`, id)
                        })
                    break
                case 'foliokiri': {
                    //if (!isPremium) return piyo.reply(from, 'Maaf, ini adalah fitur premium, untuk menggunakan fitur ini silahkan donasi, Kirim #donasi untuk melihat info donasi', id)
                ...
                case 'fb':
                    ...
                    break
                case 'instagram': //RECODE BY NUMAPU
                case 'ig':
                    try {
                ...
                case 'tiktoknowm':
                    ...
                    await piyo.sendFile(from, './media/video/tiktok2.mp4', 'numa.mp4', 'nih kak', id)
                ...
                case 'tiktok':
                    await piyo.reply(from, ind.wait(), id)
                    await piyo.sendFileFromUrl(from, `http://piyobot.cf/download.png`, 'image.jpg', `Video Ditemukan...\n\nNumaBot\n\n*_Sabar, NumaBot lagi ngirim Videonya_*`, id)
                    await piyo.sendFileFromUrl(from, `https://lolhuman.herokuapp.com/api/tiktokwm?apikey=${lolhuman}&url=${q}`, 'numa.mp4', `*Tiktok Downloader*\n*Creator :* Numapu`, id)
                    break
                ...
                case 'playv':
                    ...
                    await piyo.sendFileFromUrl(from, url_video, 'numa.mp4', 'Nih Kak', id)
                    break
                ...
                case 'ytmp4':
                    ...
                    try {
                        ...
                        await piyo.sendFile(from, `./media/${sender.id}mp4`, 'numa.mp4', 'Nih Kak', id)
                    } catch (err) {
                        console.error(err.message)
                        await piyo.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Ada Sedikit Error')
                        piyo.sendText(from, 'Fitur Error : ' + err)
...
➸ *Jumlah Postingan:* ${video}`
                ...
                case 'infomobil':
                    ...
                    piyo.reply(from, 'Sabar cuy lagi gwehj cari....\n\nKalo Kaga Muncul Ada yang salah', id)
                    try {
                ...
                case 'infomotor':
                    ...
                    piyo.reply(from, 'Sabar cuy lagi gwehj carii....\n\nKalo Kaga Muncul Ada yang salah', id)
                    try {
                ...
                case 'chika'://NumaBot
                    ...
                    fetch('https://raw.githubusercontent.com/AlvioAdjiJanuar/chika/main/chika.txt')
                        ...
                        .then(body => {
                            ...
                            piyo.sendFileFromUrl(from, `https://piyobot.000webhostapp.com/${chikax}.mp4`, 'chika.mp4', 'Nih coy', id)
                ...
                case 'nekopoiapp':
                    if (!isPremium) return piyo.reply(from, `Maaf, ini adalah fitur premium, untuk menggunakan fitur ini silahkan beli premium ke owner Murah kok`, id)
                    await piyo.reply(from, ind.wait(), id)
                    await piyo.sendFileFromUrl(from, `https://linkpoi.me/neko`, 'neko.apk', `Nih apk nya mas`, id)
                        .then(() => console.log('Success send apk'))
                    break
                case 'register':
                ...
                case 'toptt':
                    if (quotedMsg) {
                        ...
                        piyo.reply(from, `Gaada data yang direply kak`, id)
                    }
                    break
                case 'translate':
                ...
                case 'sspc':
                    ...
                    break
                case 'waktu':
                    piyo.reply(from, `*Waktu NumaBot Aktif* \n\nPAGI = 07:00 - 10:00\n\nSIANG = 13:00 - 16:00\n\nMALAM = 19:00 - 22:00`, id)
                    break
                case 'listbacot':
                    const bacul = dbcot
                ...
                //Fun Menu
                case 'chat':
                    piyo.reply(from, `Untuk mengaktifkan NumaBot chat\n\n penggunannya\n${prefix}numa on --mengaktifkan\n${prefix}numa off --menonatifkan\n`, id)
                    break
                    break
                case 'numa':
                    if (isGroupMsg) {
                        ...
                        if (args.length !== 1) return piyo.reply(from, `Untuk mengaktifkan numa pada  Chat\n\nPenggunaan\n${prefix}numa on --mengaktifkan\n${prefix}numa off --nonaktifkan\n`, id)
                        if (args[0] == 'on') {
                            chatt.push(chatId)
                            fs.writeFileSync('./settings/piyo.json', JSON.stringify(chatt))
                            piyo.reply(from, 'Mengaktifkan bot NumaBot!', id)
                        } else if (args[0] == 'off') {
                            let inxx = chatt.indexOf(chatId)
                            chatt.splice(inxx, 1)
                            fs.writeFileSync('./settings/piyo.json', JSON.stringify(chatt))
                            piyo.reply(from, 'Menonaktifkan bot NumaBot!', id)
                        } else {
                            piyo.reply(from, `Untuk mengaktifkan numa \n\nPenggunaan\n${prefix}numa on --mengaktifkan\n${prefix}numa off --nonaktifkan\n`, id)
                        }
                    }
                    break
                case 'simisimi':
                ...
                //////////////////////////////////////////////////////Owner Bot////////////////////////////////////////////////////
                case 'getses':
                    if (!isOwnerBot) return piyo.reply(dari, 'Perintah ini hanya untuk Owner NumaBot', id)
                    const sesPic = await piyo.getSnapshot()
                    piyo.sendFile(from, sesPic, 'session.png', 'Nih coy', id)
                    break
                case 'haram':
             ...
             case 'sendsewa':
                    ...
                    await piyo.sendText(`${res.owner}`, `Group Anda ${res.subject} Telah Menyewakan Bot NumaBot`,  id)
                ...
                case 'bcimg':
                    ...
                    if (isMedia && type === 'image' || isQuotedImage) {
                        ...
                        for (let grp of chaim) {
                            ...
                            if (!cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'numa.jpg', `[NUMABOT BROADCAST]\n\n${q}`, '', id)
                            if (cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'numa.jpg', `[NUMABOT BROADCAST]\n\n${q}`, '', id)
                        }
                ...
                case 'ban':
                    ...
                    if (ar[0] === 'add') {
                        if (mentionedJidList.length !== 0) {
                            ...
                            await piyo.reply(from, 'Mampus gua ban lu!', id)
                        } else {
                ...
                case 'slidingtext':
                    ...
                    await piyo.sendFileFromUrl(from, `https://api.vhtear.com/slidingtext?text=${q}&apikey=${vhtearkey}`, '', 'nih....', id)
                        .then(() => console.log('Success creting image!'))
                        .catch(async (err) => {
                            console.error(err)
                            await piyo.reply(from, `Error!`, id)
            ...
            if ((!isCmd && chatId && isChat) && message.type === 'chat') {
                ...
                await piyo.reply(from, `NumaBot Menjawab: ${piyoget.data.result}`, id)
            }
            // Simi-simi function
